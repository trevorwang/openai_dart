...
extension Transcription on OpenaiClient {
  ...
  Future<TranscriptionResponse> createTranscription(
    ...
    final bytes = request.bytesArray;
    ...
    if (file != null) {
      ...
      r.files.add(MultipartFile.fromBytes('file', bytes, filename: 'audiofile.mp3'));
    } else {
...
class TranscriptionRequest with _$TranscriptionRequest {
  const factory TranscriptionRequest({
    ...
    /// Bytes array from data buffer, in one of these formats: mp3, mp4, mpeg,
    /// mpga, m4a, wav, or webm.
    List<int>? bytesArray,
